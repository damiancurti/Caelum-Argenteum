...
The current V4.29.0n build continues the authorized V4.29 line with bounded mass-combat scheduling and the first definitive UI foundation. MAP02 keeps the exact 1,875 active AI population inside its 15,000-body main field, uses bounded straight projectiles, prevents diagnostic crowd collision and friendly fire, and leaves perception, chase and attacks under one coordinator. The supplied four-run comparison isolates the remaining abrupt stop to sustained native pursuit: Look-only remained stable, while the last run froze with main-field attacks still disabled and custom contacts at zero. Chase is therefore capped at 20 native calls per tic, and server diagnostic settings are captured once per map instead of being read or copied in the hot path. The stress-only population still avoids approximately 49,500 unnecessary anatomy/armor/status helper objects. MAP01 is restored byte-for-byte to the accepted pre-expansion V4.29.0i layout; the rejected V4.29.0l/V4.29.0m continuous-room reconstruction is removed rather than patched. The inherited Doom status bar and face remain removed, HUD-01 resource colors are accepted, and `CaelumText` keeps clearer word spacing in the Journal and secondary menus. Normal gameplay AI, validated Seal mechanics and the complete V4.27 input contract are unchanged.
