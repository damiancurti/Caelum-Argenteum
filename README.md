...
The current V4.29.0m build continues the authorized V4.29 line with bounded mass-combat scheduling and the first definitive UI foundation. MAP02 keeps the exact 1,875 active AI population inside its 15,000-body main field, uses bounded straight projectiles, prevents diagnostic crowd collision and friendly fire, and gives perception, chase and attacks live coordinator-owned controls. The stress-only population now avoids approximately 49,500 unnecessary anatomy/armor/status helper objects and bypasses per-tic RPG recomputation while retaining native states and scheduled AI; normal gameplay actors remain unchanged. MAP01 keeps eight continuous native first-floor rooms but moves both façades behind the staircase landings, producing a straight 96-MU corridor between Y=±272 and Y=±368 while preserving every 119-MU stair width. Lateral openings remain intentionally closed for visual acceptance. The inherited Doom status bar and face are removed; HUD-01 resource colors are manually accepted, and `CaelumText` uses clearer word spacing in the Journal and secondary menus. Seal mechanics remain author-validated and closed, and the complete V4.27 input matrix remains deferred until the crafting system is complete.
