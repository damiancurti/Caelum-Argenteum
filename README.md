...
The current V4.29.0q build continues the authorized V4.29 line with bounded mass-combat scheduling and the first definitive UI foundation. The 10-call Chase gate is manually stable for 368 reports without main-field attacks and 548 with full combat, across a consecutive MAP02 reload in the same process. Both runs reach all 1,983 AI targets, admit at most 10 native Chase calls per tic and 350 per simulated second, and retain zero custom contacts/callbacks; the enabled run completes 40 projectile lifecycles with at most one live missile and no spawn failure. MAP01 retains the accepted continuous T-shaped central rooms and four exact 119×119-MU stair landings. It now closes the obsolete openings in the four extreme rooms, adds four 64-MU sliding doors entirely inside their real shared walls, and preserves the two central dividers and their door openings. Stairs, passages, room footprints and every previous Thing remain fixed. The inherited Doom status bar and face remain removed, HUD-01 resource colors are accepted, and `CaelumText` keeps clearer word spacing in the Journal and secondary menus. Normal gameplay AI, validated Seal mechanics and the complete V4.27 input contract are unchanged.
