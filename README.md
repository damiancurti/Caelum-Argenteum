...
The current V4.29.0e build continues the authorized V4.29 line with a controlled mass-AI gate before crafting content changes. Seal mechanics remain author-validated and closed. The four mass-test NPC families retain the V4.29.0b bounded straight projectile path, and the contact hot path remains once-per-shared-pair/tic with reusable work objects. MAP02 restores the exact historical 1,875-AI population that previously failed with guided projectiles—125 each of Rulo, Argento, Caella, Ronnie and Bull plus 1,250 Giant Rats—while the remaining 13,125 actors in the main field stay passive. Its three 500-Rat Quintessence matrices remain physically separated by 18,000 MU, now entirely inside GZDoom's valid coordinate range, and telemetry distinguishes active-AI targets from passive actors that merely acquired a target through damage. MAP01 no longer uses finite-panel repairs for the central first floor: each mirrored pair is one clean 352×352-MU native perimeter divided by one 8-MU wall with a 64-MU internal doorway. All six leaves slide parallel to their vertical walls and the door-target sectors receive both the first-floor slab and continuous roof slab. V4.29.0d's compact, bilateral side table remains unchanged. The complete V4.27 input matrix remains explicitly deferred until the crafting system is complete.
