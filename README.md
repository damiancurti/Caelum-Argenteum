...
The current V4.29.0b build continues the authorized V4.29 line with a parallel physics-diagnostic gate before crafting content changes. Seal mechanics remain author-validated and closed. The four mass-test NPC families now fire bounded straight elemental projectiles with no homing search or radial explosion; the explosive form remains only in its isolated comparison room and for systems that explicitly require it. Caelum resolves a shared contact pair at most once per tic, rejects non-closing callbacks before allocating or normalizing bodies, and reuses per-actor physics work objects. MAP02 retains the original 15,000 passive stress actors and supplies nine isolated rooms, including three equal 500-Rat Quintessence groups that compare native collision, complete Caelum contacts and same-species pass-through. Its monitor separates unique pair work, duplicate callbacks and resting callbacks. In MAP01, the four central first-floor finite panels now cover the complete 72-MU spans and offset their reverse face only along the wall normal, closing the visible interior endpoint gaps. The complete V4.27 input matrix remains explicitly deferred until the crafting system is complete.
