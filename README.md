...
The current MAP01 construction prototype is based on the stable pre-gate V4.26.5r layout. It retains eight validated trap-door rooms and aligned staircase pairs, restores the finite walls beside the rear-room entrance, and roofs only the four closed regions behind the intermediate staircases. The complete central corridor remains open to the sky, with no rejected terrace partition. The exterior test field spans 60,000 × 60,000 MU and every real sky sector reaches Z=30000. All 3D-floor control sectors reside outside the playable perimeter. One isolated first-floor room uses independent 3D-floor floor, roof, wall and rising-door volumes. It keeps a 96-MU perimeter circulation inset, reserving 8 MU for finite railings and 88 MU clear passage for the maximum player body; seven-room replication remains gated by manual validation.

Both rear staircases now terminate in 119-MU-wide roof passages integrated into the rear room. Their landings and continuations belong to the same target-100 roof system as that room, giving it a deliberate non-rectangular footprint instead of placing independent raised sectors against its walls. Exposed edges use finite 3D middle walls and the exterior ground remains a separate flat sector.

The current actor baseline centralizes derived Health and temporary effective-mass changes. The bull uses authored Speed 10 modified by Agility Type-4 (approximately 10.832 at Agility 20) and doubles that derived value during its Air-driven Charge (approximately 21.663 MU/tic). Its Constitution 40 and 900-kg body now produce the player-equivalent 82800 maximum Health; effective collision mass remains 900 normally and 1800 only during Charge. The failed V4.28.0a upper-room experiment is removed from MAP01; its approved two-row `1 | 2 | 1` plan remains pending a validated upper-only 3D-floor room/door module.
...
- First hostile quadruped foundation: the bull has direction-aware head/torso/four-leg anatomy, physical/technical/social/mental attributes and project-owned eight-direction sprites.
...
V4.27.0j removes the eight 1-MU bridge edges responsible for the remaining narrow terrace projection. Giant Gauntlets become a weapon-based Block source with same-tier Buckler statistics and special defensive behavior, using their own durability and HUD image. The four reserved User inputs now emit a localized successful-use acknowledgement while their authored effects remain pending. `CaelumBull` introduces the first hostile quadruped, direction-aware front head and four-leg anatomy, real-scale mass/body dimensions and clean project-owned directional sprites.
