#!/usr/bin/env python3
"""Build the deterministic source-only Caelum Argenteum 4.32.0o patch."""
...
def load_audit(project: Path):
    audit_path = project / "tools/audit_4_32_0o.py"
    spec = importlib.util.spec_from_file_location("caelum_audit_4_32_0o", audit_path)
...
def main() -> None:
    ...
    baseline = args.baseline_0f.resolve()
    baseline_n = args.baseline_0n.resolve()
    output = args.output.resolve()
    audit = load_audit(project)

    audit_command = [
        sys.executable,
        str(project / "tools/audit_4_32_0o.py"),
        "--project-root", str(project),
        "--baseline-0f", str(baseline),
        "--baseline-0n", str(baseline_n),
    ]
    ...
    require(
        not any(name.lower().endswith((".pk3", ".wad")) for name in entries),
