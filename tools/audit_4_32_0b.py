#!/usr/bin/env python3
"""Deterministic source, map and package audit for Caelum 4.32.0b."""
...
def check_map(runtime: Path, baseline: Path) -> None:
    ...
    original_text = original[1][1].decode("utf-8")
    require(current_text.count(PALOMO_BLOCK) == 1, "Palomo block is not exact")
    require(current_text.replace(PALOMO_BLOCK, "") == original_text,
            "MAP01 changed beyond the Palomo block")
...
def check_sources(runtime: Path) -> None:
    ...
    icon = runtime / next(iter(NEW_FILES))
...
def check_candidate(runtime: Path, base_pk3: Path, candidate_pk3: Path) -> None:
    with zipfile.ZipFile(base_pk3) as base, zipfile.ZipFile(candidate_pk3) as candidate:
        ...
        require(set(names) == set(base_names) | NEW_FILES, "candidate PK3 entry set changed")
        require(not (NEW_FILES & set(base_names)), "new file already exists in base PK3")
        for name in names:
...
def main() -> None:
    ...
    print("Caelum 4.32.0b audit passed")
    print(f"Runtime delta: {len(CHANGED_EXISTING)} changed + {len(NEW_FILES)} new files")
    print("MAP01: one exact anchored Palomo block; all other map bytes preserved")
