"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0am.
...
MAP01_UPDATED_COUNTS = (927, 1286, 2362, 382, 227)
...
# una intencion vertical explicita a cada pieza. Todos los sectores de la
# plataforma oriental conservan la losa transitable z=256..264.
...
# El corredor sigue siendo el recorrido directo entre puerta y descanso, pero
# ya no sustituye la plataforma: el suelo lateral aceptado se conserva.
...
def rebuild_ground_and_balconies(blocks) -> None:
    ...
    # Retira los perfiles heredados y reinstala de forma explicita la losa y
    # las paredes de la ampliacion. No se borra la plataforma transitable.
    for sector_index in LEGACY_UPPER_ONLY_SECTORS:
        blocks["sector"][sector_index]["id"] = str(UPPER_FLOOR_ONLY_TAG)
...
def validate_map01_architecture(blocks) -> None:
    ...
    # La plataforma central conserva su suelo a z=256..264; las dos franjas de
    # balcon laterales comprobadas arriba permanecen deliberadamente abiertas.
    for point in ((1300.0, 100.0), (1400.0, 100.0), (1400.0, -200.0),
                  (1600.0, 200.0), (1600.0, -200.0)):
        profile = physical_profile_at(blocks, *point)
        if not profile_covers(profile, 256, 264):
            raise ValueError(f"Falta plataforma superior en {point}")
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0am inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0am inesperado: {digest}")
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0am inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0am inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: balcon exterior y techo a dos aguas reconstruidos en 4.29.0am"
        if changed01
        else "MAP01: arquitectura 4.29.0am ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0am verificadas"
        if changed02
