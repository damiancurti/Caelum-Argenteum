#!/usr/bin/env python3
"""Audita la economía y la Caja Mágica de Caelum 4.32.0a-r4."""
...
def check_economy_source(runtime: Path) -> None:
    ...
    # CaelumEconomyRules hereda de Object, por lo que sus métodos sin
    # calificador explícito se compilan en data scope. Estas tres rutinas
    # inspeccionan actores Inventory vivos y llaman métodos play; GZDoom
    # 4.14.2 rechaza esa combinación durante LoadActors.
    for helper in (
        "GetEquipmentItemBaseValue",
        "GetInventoryUnitBaseValue",
        "GetInventoryStackBaseValue",
    ):
        require(
            re.search(
                rf"(?m)^\s*play\s+static\s+double\s+{helper}\s*\(",
                economy,
            ) is not None,
            f"Helper de inventario fuera de play scope: {helper}",
        )
...
def main() -> None:
    ...
    print("4.32.0a-r4 audit passed")
    print("zscript: live Inventory pricing helpers = play scope")
