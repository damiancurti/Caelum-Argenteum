"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0ak.
...
continuo, elimina los retornos que cortaban los balcones y cierra las dos
esquinas inferiores que enlazan el edificio antiguo con la ampliacion.
...
MAP01_UPDATED_COUNTS = (932, 1289, 2360, 382, 227)
...
def validate_map01_architecture(blocks) -> None:
    ...
    # El perimetro exterior vuelve a ser continuo, incluidas las dos uniones
    # de 8x8 MU que faltaban junto a la edificacion antigua.
    perimeter_points = (
        (1205.0, -387.0),
        (1205.0, 387.0),
        (1400.0, -387.0),
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0ak inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0ak inesperado: {digest}")
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0ak inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0ak inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: habitacion 150 MU al oeste, balcones libres y esquinas 4.29.0ak aplicados"
        if changed01
        else "MAP01: arquitectura 4.29.0ak ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0ak verificadas"
        if changed02
