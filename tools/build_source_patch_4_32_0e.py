#!/usr/bin/env python3
"""Build the deterministic, source-only Caelum Argenteum 4.32.0e patch."""
...
def load_audit(project: Path):
    audit_path = project / "tools/audit_4_32_0e.py"
    spec = importlib.util.spec_from_file_location("caelum_audit_4_32_0e", audit_path)
...
def main() -> None:
    ...
    audit_command = [
        sys.executable,
        str(project / "tools/audit_4_32_0e.py"),
        "--runtime-root", str(runtime),
        "--baseline-runtime", str(baseline),
        "--project-root", str(project),
    ]
    if args.supplied_assets is not None:
        audit_command.extend(["--supplied-assets", str(args.supplied_assets.resolve())])
    subprocess.run(audit_command, check=True)

    entries: dict[str, Path] = {
        f"src/{relative}": runtime / relative
        for relative in audit.expected_runtime_delta()
    }
    ...
    package_audit = audit_command + ["--package", str(output)]
    subprocess.run(package_audit, check=True)
