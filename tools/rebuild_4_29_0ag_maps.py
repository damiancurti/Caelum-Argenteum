"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0ag.
...
913/914. Desplaza la habitacion superior 104 MU hacia la entrada principal,
cierra todo su perimetro y completa la losa oriental. Los retornos de ambos
balcones vuelven a ser paredes solo debajo del piso transitable.
...
MAP01_UPDATED_COUNTS = (898, 1231, 2252, 362, 225)
...
PLATFORM_GROUND_WALL_TAG = 542
PLATFORM_BALCONY_RETURN_TAG = 543
PLATFORM_FULL_WALL_TAG = 544
PLATFORM_UPPER_WALL_TAG = 545
PLATFORM_BALCONY_TAG = 546
PLATFORM_FLOOR_ONLY_TAG = 547
ROOM_PROFILE_TAG_START = 550
...
# Los retornos 160/161 cierran el costado que seguia abierto en 0ae. Las
# superficies 181/182 son paredes solo debajo del balcon; 183..186 son el
# espacio habitable inferior y las dos losas transitables superiores.
FULL_RETURN_WALL_SECTORS = (160, 161)
FIRST_FLOOR_INNER_WALL_SECTORS = (177, 178, 179, 180)
BALCONY_RETURN_WALL_SECTORS = (181, 182)
OPEN_BALCONY_SECTORS = (183, 184, 185, 186)
GROUND_OUTER_WALL_SECTORS = (156, 158, 167, 168, 169, 170, 187, 188)
PLATFORM_FLOOR_ONLY_SECTORS = (70, 71)

# El recinto mantiene 1426 x 782 MU, pero se desplaza 104 MU al oeste. Su
# borde oriental termina exactamente en X=1201, sobre la pared estructural,
# y ya no queda suspendido sobre el corredor exterior. El perimetro es cerrado.
ROOM_OUTER = (-225.0, -391.0, 1201.0, 391.0)
ROOM_INNER = (-217.0, -383.0, 1193.0, 383.0)
EXPECTED_ROOM_RECTANGLES = 179
...
FIXED_PLATFORM_PROFILES: dict[int, Profile] = {
    PLATFORM_GROUND_WALL_TAG: (
        (0, 128, MANSION_WALL, MANSION_WALL),
        SECOND_FLOOR_RANGE,
    ),
    PLATFORM_BALCONY_RETURN_TAG: (
        (0, 128, MANSION_WALL, MANSION_WALL),
        GROUND_FLOOR_RANGE,
        SECOND_FLOOR_RANGE,
    ),
    PLATFORM_FULL_WALL_TAG: (
        (0, 256, MANSION_WALL, MANSION_WALL),
        SECOND_FLOOR_RANGE,
    ),
    PLATFORM_UPPER_WALL_TAG: (
        (128, 256, MANSION_WALL, MANSION_WALL),
        SECOND_FLOOR_RANGE,
    ),
    PLATFORM_BALCONY_TAG: (
        GROUND_FLOOR_RANGE,
        SECOND_FLOOR_RANGE,
    ),
    PLATFORM_FLOOR_ONLY_TAG: (SECOND_FLOOR_RANGE,),
}
...
def add_profile_controls(blocks, profile_tags: dict[Profile, int]) -> None:
    slot = 0
...
def rebuild_ground_and_balconies(blocks) -> None:
    ...
    for sector_index in GROUND_OUTER_WALL_SECTORS:
        blocks["sector"][sector_index]["id"] = str(PLATFORM_GROUND_WALL_TAG)
    for sector_index in FULL_RETURN_WALL_SECTORS:
        blocks["sector"][sector_index]["id"] = str(PLATFORM_FULL_WALL_TAG)
    for sector_index in GROUND_DIVIDER_WALL_SECTORS:
        blocks["sector"][sector_index]["id"] = str(PLATFORM_GROUND_WALL_TAG)
    for sector_index in FIRST_FLOOR_INNER_WALL_SECTORS:
        blocks["sector"][sector_index]["id"] = str(PLATFORM_UPPER_WALL_TAG)
    for sector_index in BALCONY_RETURN_WALL_SECTORS:
        blocks["sector"][sector_index]["id"] = str(PLATFORM_BALCONY_RETURN_TAG)
    for sector_index in OPEN_BALCONY_SECTORS:
        blocks["sector"][sector_index]["id"] = str(PLATFORM_BALCONY_TAG)
    for sector_index in PLATFORM_FLOOR_ONLY_SECTORS:
        blocks["sector"][sector_index]["id"] = str(PLATFORM_FLOOR_ONLY_TAG)

    add_fixed_platform_controls(blocks)
    for rectangle in GROUND_DIVIDER_SPLITS:
        carve_from_host(blocks, rectangle, 107, PLATFORM_GROUND_WALL_TAG)
...
def add_centered_second_floor_room(blocks) -> dict[Profile, int]:
    ...
    add_profile_controls(blocks, profile_tags)
    return profile_tags
...
def physical_profile_at(blocks, x: float, y: float) -> Profile:
    sector_index = sector_at_point(blocks, x, y)
    tag = integer(blocks["sector"][sector_index], "id")
    return control_profile(blocks, tag)
...
def validate_map01_architecture(blocks) -> None:
    ...
    if groups[NEW_DIVIDER_GROUP] != 2 or groups[915] != 0:
    ...
    # Pruebas fisicas por punto: cierre lateral, divisor, vano y balcones.
    for point in ((1205.0, -287.0), (1205.0, 287.0)):
        profile = physical_profile_at(blocks, *point)
        if not profile_covers(profile, 0, 256):
            raise ValueError(f"El retorno lateral sigue abierto en {point}")
        if not profile_covers(profile, 256, 264):
            raise ValueError(f"Falta piso superior sobre el retorno {point}")
    ...
    for point in ((1310.0, -340.0), (1310.0, 340.0)):
        profile = physical_profile_at(blocks, *point)
        if (
            not profile_covers(profile, 128, 136)
            or profile_covers(profile, 136, 256)
            or not profile_covers(profile, 256, 264)
        ):
            raise ValueError(f"El balcon quedo bloqueado en {point}")

    # Todas las celdas del rectangulo superior reciben piso y techo; la franja
    # perimetral completa recibe pared 264..392.
    outer_minimum_x, outer_minimum_y, outer_maximum_x, outer_maximum_y = ROOM_OUTER
    inner_minimum_x, inner_minimum_y, inner_maximum_x, inner_maximum_y = ROOM_INNER
    samples = []
    x_samples = (
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0ag inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0ag inesperado: {digest}")
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0ag inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0ag inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: plataforma y segundo piso 4.29.0ag aplicados"
        if changed01
        else "MAP01: arquitectura 4.29.0ag ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0ag aplicadas"
        if changed02
