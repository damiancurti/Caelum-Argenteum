"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0al.
...
continuo, elimina los retornos y los dos muros terminales que cortaban los
balcones, y cierra las dos esquinas inferiores que enlazan el edificio antiguo
con la ampliacion. Los muros terminales conservan cerrado solo el volumen de
...
conserva su puerta doble y alarga el corredor hasta la escalera. El terreno
exterior conserva el pasto original.
...
MAP01_UPDATED_COUNTS = (940, 1297, 2368, 384, 227)
...
def validate_map01_architecture(blocks) -> None:
    ...
    # El perimetro de planta baja sigue cerrado, incluidas las dos uniones de
    # 8x8 MU. El frente central oriental conserva su pared de dos plantas.
    perimeter_points = (
        (1205.0, -387.0),
        (1205.0, 387.0),
        (1400.0, -387.0),
    ...
    # Las dos paredes terminales conservan la fachada inferior, pero no pueden
    # invadir z=136..256 ni volver a cubrir el balcon desde z=256.
    for point in ((1693.0, -340.0), (1693.0, 340.0)):
        profile = physical_profile_at(blocks, *point)
        ...
        if not profile_covers(profile, 128, 136):
            raise ValueError(f"Falta piso en el extremo del balcon {point}")
        if profile_covers(profile, 136, 256):
            raise ValueError(f"La pared terminal sigue bloqueando {point}")
        if profile_covers(profile, 256, 264):
            raise ValueError(f"El extremo del balcon quedo cubierto en {point}")
...
def validate_map01_surface_set(blocks) -> None:
    ...
    if blocks["sector"][WORLD_SECTOR].get("texturefloor") != MANSION_GRASS:
        raise ValueError("El terreno exterior de MAP01 no usa CMGR01")
    if any(
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0al inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0al inesperado: {digest}")
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0al inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0al inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: muros terminales del balcon abiertos en 4.29.0al"
        if changed01
        else "MAP01: arquitectura 4.29.0al ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0al verificadas"
        if changed02
