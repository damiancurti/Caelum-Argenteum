"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0af.

MAP01 parte exclusivamente de la base aceptada 4.29.0ad. La reconstruccion
descarta las habitaciones provisionales de 0ae y conserva el acceso oriental,
sus escaleras, el tunel y el porton 913. Cierra de forma simetrica las alas de
planta baja, abre el paso real a ambos balcones, reemplaza las dos aletas por
un divisor completo con una puerta doble y levanta una unica habitacion
centrada en el segundo piso.
...
# superficies 181/182 dejan de bloquear el acceso a los balcones.
FULL_RETURN_WALL_SECTORS = (160, 161)
FIRST_FLOOR_INNER_WALL_SECTORS = (177, 178, 179, 180)
OPEN_BALCONY_SECTORS = (181, 182, 183, 184, 185, 186)
GROUND_OUTER_WALL_SECTORS = (156, 158, 167, 168, 169, 170, 187, 188)

# Habitacion estructural centrada sobre el cuerpo principal. Mide 1426 x 782
# MU, mas de tres veces el largo del recinto oriental rechazado de 0ae. El
# muro tiene 8 MU y conserva un acceso oriental de 128 MU al descanso.
ROOM_OUTER = (-121.0, -391.0, 1305.0, 391.0)
ROOM_INNER = (-113.0, -383.0, 1297.0, 383.0)
ROOM_EAST_OPENING = (-64.0, 64.0)
...
def rebuild_ground_and_balconies(blocks) -> None:
    ...
    # Control independiente: pared solo en planta baja, sin cerrar el balcon.
    add_control(
        blocks,
        GROUND_WALL_TAG,
    ...
    for rectangle in GROUND_DIVIDER_SPLITS:
        carve_from_host(blocks, rectangle, 107, GROUND_WALL_TAG)
...
def room_rectangles(blocks) -> list[RoomRectangle]:
    ...
    for minimum_y, maximum_y in zip(y_coordinates, y_coordinates[1:]):
        ...
        for minimum_x, maximum_x in zip(x_coordinates, x_coordinates[1:]):
            ...
            if (
                ...
                and ROOM_EAST_OPENING[0] < center_y < ROOM_EAST_OPENING[1]
            ):
                kind = "interior"
            cells.append(
                (
    ...
    if len(rectangles) != EXPECTED_ROOM_RECTANGLES:
        raise ValueError(
...
def validate_map01_architecture(blocks) -> None:
    ...
    if control_profile(blocks, GROUND_WALL_TAG) != (
        (0, 128, MANSION_WALL, MANSION_WALL),
    ):
        raise ValueError("La pared de planta baja no conserva 0..128 MU")

    # Pruebas fisicas por punto: cierre lateral, divisor, vano y balcones.
    for point in ((1205.0, -287.0), (1205.0, 287.0)):
        if not profile_covers(physical_profile_at(blocks, *point), 0, 256):
    ...
    if profile_covers(physical_profile_at(blocks, 1205.0, 0.0), 0, 128):
        raise ValueError("El divisor cerro el vano de su puerta")
    for point in ((1310.0, -340.0), (1310.0, 340.0)):
        profile = physical_profile_at(blocks, *point)
        if not profile_covers(profile, 128, 136) or profile_covers(profile, 136, 256):
            raise ValueError(f"El balcon quedo bloqueado en {point}")

    # Todas las celdas del rectangulo superior reciben piso y techo; solo la
    # franja perimetral, salvo el vano oriental, recibe pared 264..392.
    outer_minimum_x, outer_minimum_y, outer_maximum_x, outer_maximum_y = ROOM_OUTER
    inner_minimum_x, inner_minimum_y, inner_maximum_x, inner_maximum_y = ROOM_INNER
    samples = []
    x_samples = (
    ...
    for x, y in samples:
        ...
        if x > inner_maximum_x and ROOM_EAST_OPENING[0] < y < ROOM_EAST_OPENING[1]:
            wall_expected = False
    ...
    if len(room_targets) != EXPECTED_ROOM_RECTANGLES:
        raise ValueError(
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0af inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0af inesperado: {digest}")
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0af inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0af inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: balcones, divisor y segundo piso 4.29.0af aplicados"
        if changed01
        else "MAP01: arquitectura 4.29.0af ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0af aplicadas"
        if changed02
