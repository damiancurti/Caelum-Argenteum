"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0aj.
...
los balcones. Desplaza la habitacion superior exactamente 100 MU al oeste,
conserva su puerta doble, mantiene la plataforma central y vuelve a descubrir
los dos balcones laterales. El terreno exterior recibe pasto original.
...
MAP01_UPDATED_COUNTS = (976, 1335, 2412, 396, 227)
...
# El recinto mantiene 1426 x 782 MU y se desplaza exactamente 100 MU al oeste
# respecto de 0ai. La puerta sigue centrada en su costado oriental.
ROOM_OUTER = (-221.0, -391.0, 1205.0, 391.0)
ROOM_INNER = (-213.0, -383.0, 1197.0, 383.0)
ROOM_EAST_DOORWAY = (-64.0, 64.0)
EXPECTED_ROOM_RECTANGLES = 193
...
UPPER_CORRIDOR = (1205.0, -64.0, 1697.0, 64.0)
EXPECTED_CORRIDOR_RECTANGLES = 7
...
FIXED_PLATFORM_PROFILES: dict[int, Profile] = {
    ...
    FIRST_FLOOR_WALL_TAG: (
        (128, 256, MANSION_WALL, MANSION_WALL),
        SECOND_FLOOR_RANGE,
    ),
    OUTER_PERIMETER_TAG: (
...
def validate_map01_architecture(blocks) -> None:
    ...
    # Los dos balcones de 96 MU conservan su piso, espacio libre y cielo abierto.
    for point in (
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0aj inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0aj inesperado: {digest}")
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0aj inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0aj inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: habitacion oeste, balcones abiertos y pasto 4.29.0aj aplicados"
        if changed01
        else "MAP01: arquitectura 4.29.0aj ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0aj aplicadas"
        if changed02
