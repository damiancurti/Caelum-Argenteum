"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0an.
...
0an conserva ese recinto y su techo inclinado. En el ala meridional agrega
una pared de primer piso que nace en la esquina interior de la base del primer
peldaño y llega hasta la pared longitudinal x=1201..1209. Retira la losa plana
z=256..264 de la ampliación x=1201..1697; la escalera, el corredor central de
128 MU y el techo a dos aguas aprobado no se alteran. Esa cubierta lateral
queda deliberadamente abierta para reconstruirla en un incremento posterior.
...
MAP01_UPDATED_COUNTS = (907, 1266, 2342, 377, 231)
...
# Se eliminan primero los perfiles heredados de la ampliación. Luego se asigna
# una intención vertical explícita a cada pieza. La losa z=256..264 desaparece
# fuera del cuarto y del corredor central que se reconstruyen más adelante.
...
# Cuatro paneles de 124 MU cubren exactamente x=1201..1697. Su eje queda cuatro
# MU al sur de la base y=-320; los bloqueadores tocan el peldaño sin cruzarlo.
SOUTH_STAIR_WALL_Y = -324.0
SOUTH_STAIR_WALL_START_X = 1201.0
SOUTH_STAIR_WALL_PANEL_WIDTH = 124
SOUTH_STAIR_WALL_PANEL_COUNT = 4
...
def make_south_stair_wall_panels() -> list[OrderedDict[str, str]]:
    """Crea una pared finita de 496 MU sin invadir el primer peldaño."""

    panels = []
    for index in range(SOUTH_STAIR_WALL_PANEL_COUNT):
        x = SOUTH_STAIR_WALL_START_X + (
            index + 0.5
        ) * SOUTH_STAIR_WALL_PANEL_WIDTH
        panels.append(
            OrderedDict(
                (
                    ("x", f"{x:.1f}"),
                    ("y", f"{SOUTH_STAIR_WALL_Y:.1f}"),
                    ("height", "136.0"),
                    ("angle", "90"),
                    ("type", str(FINITE_WALL_DOOMEDNUM)),
                    ("arg0", "0"),
                    ("arg1", str(SOUTH_STAIR_WALL_PANEL_WIDTH)),
                    ("skill1", "true"),
                    ("skill2", "true"),
                    ("skill3", "true"),
                    ("skill4", "true"),
...
def rebuild_ground_and_balconies(blocks) -> None:
    ...
    # Retira los perfiles heredados y reinstala de forma explícita paredes y
    # piso del primer nivel. La losa plana z=256..264 desaparece; después se
    # reconstruyen únicamente el cuarto aprobado y su corredor central.
...
def validate_map01_architecture(blocks) -> None:
    ...
    # Toda la U exterior conserva la fachada de planta baja, pero no puede
    # invadir z=136..256 ni volver a cubrir el balcon desde z=256.
    for point in (
        (1400.0, -387.0), (1400.0, 387.0),
        (1693.0, -340.0), (1693.0, -200.0),
        (1693.0, 200.0), (1693.0, 340.0),
    ):
        profile = physical_profile_at(blocks, *point)
    ...
    # Los dos balcones de 96 MU y toda su transicion interior conservan piso,
    # espacio libre y cielo abierto. Ningun retorno puede estrechar el paso.
    for point in (
        (1310.0, -340.0),
    ...
    for point in (
        ...
        profile = physical_profile_at(blocks, *point)
        if not profile_covers(profile, 128, 136):
            raise ValueError(f"Falta piso en la transicion del balcon {point}")
        if profile_covers(profile, 136, 256):
            raise ValueError(f"El paso al balcon quedo cerrado en {point}")
        if profile_covers(profile, 256, 264):
            raise ValueError(f"El paso al balcon quedo techado en {point}")

    # El divisor inferior conserva su vano. Fuera del corredor central ya no
    # queda la losa exterior heredada sobre sus tramos sólidos.
    for point in (
        ...
        if profile_covers(profile, 256, 264):
            raise ValueError(f"Persiste losa superior sobre el divisor {point}")
    ...
    # La antigua plataforma oriental desaparece completa. Sólo el corredor
    # central conserva z=256..264 entre el cuarto y el descanso.
    for point in (
        (1300.0, 100.0), (1400.0, 100.0),
        (1400.0, -200.0), (1600.0, 200.0), (1600.0, -200.0),
    ):
        profile = physical_profile_at(blocks, *point)
        if profile_covers(profile, 256, 264):
            raise ValueError(f"Persiste la plataforma superior en {point}")
    for point in ((1320.0, 0.0), (1500.0, 0.0), (1693.0, 0.0)):
        if not profile_covers(physical_profile_at(blocks, *point), 256, 264):
            raise ValueError(f"Falta el corredor superior en {point}")

    for point in (
        (1250.0, -300.0), (1400.0, -300.0), (1600.0, -300.0),
        (1250.0, -240.0), (1400.0, -240.0), (1600.0, -240.0),
        (1250.0, -160.0), (1400.0, -160.0), (1600.0, -160.0),
        (1250.0, -80.0), (1400.0, -80.0), (1600.0, -80.0),
        (1250.0, 80.0), (1400.0, 80.0), (1600.0, 80.0),
        (1250.0, 160.0), (1400.0, 160.0), (1600.0, 160.0),
    ):
        if profile_covers(physical_profile_at(blocks, *point), 256, 264):
            raise ValueError(f"Persiste la losa plana oriental en {point}")

    # La pared finita nace en x=1697, y=-320 y llega a la pared longitudinal
    # x=1201. Sus cuatro paneles forman 496 MU exactos sin cruzar la escalera.
    actual_wall_panels = {
        (
            float(thing["x"]),
            float(thing["y"]),
        ...
        for thing in blocks["thing"]
        if integer(thing, "type") == FINITE_WALL_DOOMEDNUM
        and float(thing["y"]) == SOUTH_STAIR_WALL_Y
    }
    expected_wall_panels = {
        (
            SOUTH_STAIR_WALL_START_X
            + (index + 0.5) * SOUTH_STAIR_WALL_PANEL_WIDTH,
            SOUTH_STAIR_WALL_Y,
            136.0,
            90,
            0,
    ...
    if actual_wall_panels != expected_wall_panels:
        raise ValueError(f"Paneles de pared meridional: {actual_wall_panels}")
    if (
        SOUTH_STAIR_WALL_START_X
        + SOUTH_STAIR_WALL_PANEL_COUNT * SOUTH_STAIR_WALL_PANEL_WIDTH
        != 1697.0
    ):
        raise ValueError("La pared meridional no termina en la base del peldaño")
    first_step = physical_profile_at(blocks, 1750.0, -304.0)
    if not profile_covers(first_step, 136, 152):
        raise ValueError("El primer peldaño meridional perdió su volumen")
    if profile_covers(first_step, 136, 256):
        raise ValueError("La pared nueva bloquea el primer peldaño meridional")
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0an inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0an inesperado: {digest}")
...
def rebuild_map01(path: Path = MAP01) -> bool:
    ...
    blocks["thing"].extend(make_south_stair_wall_panels())
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0an inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0an inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: pared meridional y retiro de losa aplicados en 4.29.0an"
        if changed01
        else "MAP01: arquitectura 4.29.0an ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0an verificadas"
        if changed02
