"""Reconstruye MAP01 y MAP02 para el candidato V4.29.0ai.
...
los balcones. Devuelve la habitacion superior a la linea estructural posterior
X=1305, conserva su puerta doble y restaura la plataforma oriental. La franja
occidental abandonada por el desplazamiento no se vuelve a crear.
...
MAP01_UPDATED_COUNTS = (950, 1282, 2302, 373, 227)
...
# El recinto mantiene 1426 x 782 MU y vuelve a la linea estructural X=1305.
# Esto lo desplaza 40 MU mas hacia el fondo respecto de 0ah y 104 MU respecto
# de la posicion rechazada de 0ag. La puerta sigue centrada en el costado este.
ROOM_OUTER = (-121.0, -391.0, 1305.0, 391.0)
ROOM_INNER = (-113.0, -383.0, 1297.0, 383.0)
ROOM_EAST_DOORWAY = (-64.0, 64.0)
EXPECTED_ROOM_RECTANGLES = 180
...
UPPER_CORRIDOR = (1305.0, -64.0, 1697.0, 64.0)
EXPECTED_CORRIDOR_RECTANGLES = 5
...
def validate_map01_architecture(blocks) -> None:
    ...
    # Cada balcon conserva ambos pisos, espacio libre y un paso central de 64 MU.
    for point in ((1400.0, -340.0), (1400.0, 340.0)):
        ...
        if not profile_covers(profile, 256, 264):
            raise ValueError(f"Falta el piso superior sobre {point}")
    ...
    # La plataforma oriental completa conserva su suelo a z=256..264. El
    # corredor central es el acceso directo, no el unico fragmento transitable.
...
def validate_map01(path: Path, blocks) -> None:
    if MAP01_UPDATED_COUNTS is not None and map_counts(blocks) != MAP01_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP01 4.29.0ai inesperada: {map_counts(blocks)}")
    ...
    if MAP01_UPDATED_SHA256 is not None and digest != MAP01_UPDATED_SHA256:
        raise ValueError(f"Hash MAP01 4.29.0ai inesperado: {digest}")
...
def validate_map02(path: Path, blocks) -> None:
    if map_counts(blocks) != MAP02_UPDATED_COUNTS:
        raise ValueError(f"Estructura MAP02 4.29.0ai inesperada: {map_counts(blocks)}")
    ...
    if MAP02_UPDATED_SHA256 is not None and digest != MAP02_UPDATED_SHA256:
        raise ValueError(f"Hash MAP02 4.29.0ai inesperado: {digest}")
...
def main() -> None:
    ...
    print(
        "MAP01: plataforma restaurada y segundo piso 4.29.0ai aplicados"
        if changed01
        else "MAP01: arquitectura 4.29.0ai ya presente"
    )
    print(
        "MAP02: superficies completas de alcantarilla 4.29.0ai aplicadas"
        if changed02
