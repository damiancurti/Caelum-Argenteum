...
The V4.28.0a mansion-floor prototype is rejected and removed in V4.28.0b. Its approved topology remains unchanged as a future target: eight smaller rooms across the six lateral support volumes, a `1 | 2 | 1` distribution per north/south row, `corridor ← A ↔ B ↔ C ↔ D → corridor` connectivity and a 119-MU circulation setback. V4.28.0d contains the first replacement module over the northwestern lateral room, built from independent 3D-floor floor, roof, wall and moving-door volumes. Upper panels use the rising-door contract: their lower plane rises into the panel's upper plane, waits and returns, without borrowing a room or ground-floor support plane. Only after that module passes repeated two-sided door use and proves that no lower-floor actor or surface is affected may it be replicated into the remaining seven rooms.
