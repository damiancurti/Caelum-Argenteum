...
The V4.28.0a mansion-floor prototype is rejected and removed in V4.28.0b. Its approved topology remains unchanged as a future target: eight smaller rooms across the six lateral support volumes, a `1 | 2 | 1` distribution per north/south row and `corridor ← A ↔ B ↔ C ↔ D → corridor` connectivity. V4.28.0d contains the first replacement module over the northwestern lateral room, built from independent 3D-floor floor, roof, wall and moving-door volumes. V4.28.0f establishes a 96-MU perimeter inset for every upper room: 8 MU reserved for finite railings and 88 MU clear circulation for the maximum 42.667-MU-diameter player. Upper panels retain the rising-door contract until a finite-height lateral slider passes an isolated stacked-floor test. Only after the module passes repeated two-sided door use and proves that no lower-floor actor or surface is affected may it be replicated into the remaining seven rooms.
