...
|---|---|---|---|---|---|---|
| Bull quadruped | `BULLA1`–`BULLH8`, `BULLI0`–`BULLN0` | Damian Curti / project-directed generation | Corrected author-supplied bull atlas | Project-owned original | Yes | 64 directional state frames plus six-frame death. |

## V4.27.0i authoritative HUD face

`graphics/caelum/face/ca_player_face.png` is the single authoritative player-face source. GZDoom's `Player.Face "CAF"` contract requires the native `CAFST`, `CAFTL`, `CAFTR`, `CAFEVL`, `CAFKILL`, `CAFOUCH`, `CAFGOD` and `CAFDEAD` status lumps under `graphics`; these are compatibility aliases regenerated byte-for-byte from that master, not separate artwork. This patch replaces every alias from the master and removes the regressed armor image from the face-state set.
